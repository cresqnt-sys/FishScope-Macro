...
class UpdateChecker(QThread):
    ...
    def is_newer_version(self, latest, current):
        try:
            # Use packaging.version to properly compare semver and pre-release tags.
            return parse_version(latest) > parse_version(current)
        except Exception:
            # Fallback: compare numeric parts only, ignoring any suffix (e.g., '-Beta1')
            try:
                def numeric_parts(v):
                    base = v.split('-')[0]
